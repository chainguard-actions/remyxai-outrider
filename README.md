...
- **[huggingface/pytorch-image-models #2767](https://github.com/huggingface/pytorch-image-models/pull/2767)** — EfficientViM backbone family ([arXiv:2411.15241](https://arxiv.org/abs/2411.15241)), **merged**. *Match:* timm's sub-25M efficient-backbone lane. *Shape:* the HSM-SSD ("Mamba duality") core reduces to pure PyTorch (no `mamba_ssm` / custom CUDA); resolved the family's long-standing Google-Drive-only weights gap ([mlvlab/EfficientViM#1](https://github.com/mlvlab/EfficientViM/issues/1)) by converting the 12 checkpoints to safetensors on the HF Hub.

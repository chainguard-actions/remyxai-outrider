...
def main() -> int:
    ...
    # Sanitize values before writing to GITHUB_ENV: strip newlines and
    # carriage returns to prevent environment-variable injection via
    # caller-supplied inputs (e.g. model, model-base-url).
    lines = [f"{k}={v.replace(chr(10), '').replace(chr(13), '')}" for k, v in routing.env.items()]
    if github_env:
